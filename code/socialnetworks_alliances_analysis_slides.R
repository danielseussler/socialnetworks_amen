...
# Saved as figure
par(mfrow = c(1, 2), mai = c(1, 0.8, 0.2, 0.0))
...
mcmc_trace(fitZNII0Hparam, facet_args = list(ncol = 3))
...
for (k in c(1, 4)) {
  # AME Model
  xlim <- range(c(fitSIXQ7Y$GOF[, k], fitZNII0H$GOF[, k], fitZNII0H$GOF[1, k])) * c(.9, 1.1)
  hist(fitZNII0H$GOF[-1, k],
       prob = TRUE, col = "yellowgreen", xlim = xlim, main = "", ylab = "",
       xlab = colnames(fitZNII0H$GOF)[k], ylim = c(0, ht[k])
  )
